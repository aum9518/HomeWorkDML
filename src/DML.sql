...
select * from cars where brand ='Hyundai';--Display all cars where brand is Hyundai
select * from cars where color = 'Red' or color='Blue';-- Display all Red and Blue cars
select * from cars where year_of_issue between 2000 and 2010;--Display all cars where issued year between 2000 and 2010
select brand,  count(brand)from cars  where brand = 'Chevrolet' group by brand ;--Display quantity of cars where car’s brand is Chevrolet
select avg(year_of_issue)from cars;--Display the average year of manufacture of cars
select * from cars where brand='Audi' or brand='Toyota' or brand='Kia' or brand='Ford';--Display the cars where brand equals to Audi, Toyota, Kia and Ford
select * from cars where model like 'T%'; --Display cars where car’s model starts with ’T’
select * from cars where model like '%e';--Display cars where car’s model ends with ‘e’;
select * from cars where brand like '_____';--Display brands where contains only 5 symbols
select sum(price) brand from cars where brand= 'Mercedes-Benz' group by brand;--Display all amounts where car's brand is Mercedes-Benz
select max(price)  from cars;--Display the most expensive car
select min(price) from cars;--Display the most the cheepest car
select * from cars where brand not in ('Toyota');--Display all cars where car's brand not TOYOTA
select *  from cars  order by price desc limit 10 offset 10;--Display 10 the most expensive cars
select * from cars order by year_of_issue desc limit 10 offset 4;--Display the newest cars between 5th to 15th
select * from cars where year_of_issue not between 1995 and 2005;--Display cars where car's year of issue not between 1995 and 2005;
select color, count(color) from cars group by color order by count(color) desc limit 1;--Display most cars in one color
